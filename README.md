...
docs/publish.md                账号策略与发布方式：为什么用独立栏目号、何时接自动化
```
...
1. **不要在云端自动发布视频号。** 登录态绑定设备与网络环境，云端 IP 异常极易触发风控。
   ...
   发布用独立的栏目号，不要用现有的个人号 —— 理由见 `docs/publish.md`。
...
- 发布：阶段一手动上传（当前）。阶段二接
  ...
  切换条件与选型理由见 `docs/publish.md`
- 待办：风格语料蒸馏（等素材）、新号注册（待回国办手机号）、本地发布助手（阶段二）、发布数据回收
